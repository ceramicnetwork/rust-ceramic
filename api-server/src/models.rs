...
/// Converts the HeapDump value to the Query Parameters representation (style=form, explode=false)
/// specified in https://swagger.io/docs/specification/serialization/
/// Should be implemented in a serde serializer
impl std::string::ToString for HeapDump {
    fn to_string(&self) -> String {
        let params: Vec<Option<String>> = vec![
            self.allocated
                .as_ref()
                .map(|allocated| ["allocated".to_string(), allocated.to_string()].join(",")),
            self.resident
                .as_ref()
                .map(|resident| ["resident".to_string(), resident.to_string()].join(",")),
        ];
...
/// Converts Query Parameters representation (style=form, explode=false) to a HeapDump value
/// as specified in https://swagger.io/docs/specification/serialization/
/// Should be implemented in a serde deserializer
impl std::str::FromStr for HeapDump {
    type Err = String;
    ...
    fn from_str(s: &str) -> std::result::Result<Self, Self::Err> {
        ...
        while key_result.is_some() {
            let val = match string_iter.next() {
                ...
                None => {
                    return std::result::Result::Err(
                        "Missing value while parsing HeapDump".to_string(),
                    )
                }
            };
            ...
            if let Some(key) = key_result {
                ...
                match key {
                    #[allow(clippy::redundant_clone)]
                    "allocated" => intermediate_rep.allocated.push(
                        <f64 as std::str::FromStr>::from_str(val).map_err(|x| x.to_string())?,
                    ),
                    #[allow(clippy::redundant_clone)]
                    "resident" => intermediate_rep.resident.push(
                        <f64 as std::str::FromStr>::from_str(val).map_err(|x| x.to_string())?,
                    ),
                    _ => {
                        return std::result::Result::Err(
                            "Unexpected key while parsing HeapDump".to_string(),
                        )
                    }
                }
            }
        ...
        // Use the intermediate representation to return the struct
        std::result::Result::Ok(HeapDump {
            allocated: intermediate_rep.allocated.into_iter().next(),
            resident: intermediate_rep.resident.into_iter().next(),
        })
    }
}

// Methods for converting between header::IntoHeaderValue<HeapDump> and hyper::header::HeaderValue

#[cfg(any(feature = "client", feature = "server"))]
impl std::convert::TryFrom<header::IntoHeaderValue<HeapDump>> for hyper::header::HeaderValue {
    type Error = String;

    fn try_from(
        hdr_value: header::IntoHeaderValue<HeapDump>,
    ) -> std::result::Result<Self, Self::Error> {
        let hdr_value = hdr_value.to_string();
        match hyper::header::HeaderValue::from_str(&hdr_value) {
            std::result::Result::Ok(value) => std::result::Result::Ok(value),
            std::result::Result::Err(e) => std::result::Result::Err(format!(
                "Invalid header value for HeapDump - value: {} is invalid {}",
                hdr_value, e
            )),
        }
    }
...
#[cfg(any(feature = "client", feature = "server"))]
impl std::convert::TryFrom<hyper::header::HeaderValue> for header::IntoHeaderValue<HeapDump> {
    type Error = String;

    fn try_from(hdr_value: hyper::header::HeaderValue) -> std::result::Result<Self, Self::Error> {
        match hdr_value.to_str() {
            std::result::Result::Ok(value) => {
                match <HeapDump as std::str::FromStr>::from_str(value) {
                    std::result::Result::Ok(value) => {
                        std::result::Result::Ok(header::IntoHeaderValue(value))
                    }
                    std::result::Result::Err(err) => std::result::Result::Err(format!(
                        "Unable to convert header value '{}' into HeapDump - {}",
                        value, err
                    )),
                }
            }
