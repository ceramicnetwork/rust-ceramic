...
pub fn ed25519_key_pair_from_secret(secret: &str) -> Result<Ed25519KeyPair> {
    ...
    let secret: [u8; 32] = secret_with_prefix
        ...
        .try_into()?;
    match parts.next() {
        None => Ok(Ed25519KeyPair::from_seed_unchecked(&secret)
        ...
        Some(public_multibase) => {
            let public_with_prefix: [u8; 34] = multibase::decode(public_multibase)
                ...
                .try_into()
                .map_err(|_| {
                    anyhow!("public key must be 0xed01 followed by 32 bytes of ed25519 public key")
                })?;
            let public: [u8; 32] = public_with_prefix
                .strip_prefix(b"\xed\x01")
                .context("public key must be 0xed01 followed by 32 bytes of ed25519 public key")?
                .try_into()?;
            Ok(
...
pub fn did_from_peer_id(peer_id_mh: &Multihash<64>) -> Result<String> {
    ...
    let ed25519_public_key = libp2p_key
        .strip_prefix(b"\x08\x01\x12\x20")
        .context("libp2p peer ID must be 0x08011220 followed by 32 bytes of ed25519 public key")?;
