...
use cid::Cid;
use recon::{
    AssociativeHash, Error as ReconError, HashCount, InsertResult, Key, Result as ReconResult,
    Sha256a,
};
...
impl CeramicOneEvent {
    ...
    pub async fn insert_many(pool: &SqlitePool, to_add: &[EventRaw]) -> Result<InsertResult> {
        ...
        for (idx, item) in to_add.iter().enumerate() {
            // let new_key = self.insert_key_int(&item.order_key, &mut conn).await?;
    ...
    /// Find the first event in a range
    pub async fn first(
        pool: &SqlitePool,
        left_fencepost: &EventId,
        right_fencepost: &EventId,
    ...
    /// Find the last event in a range
    pub async fn last(
        pool: &SqlitePool,
        left_fencepost: &EventId,
        right_fencepost: &EventId,
    ...
    /// Find the first and last event in a range
    ...
    /// Find any Event IDs that are missing values. Should be deprecated with the move to simultaneous keys/values.
    pub async fn keys_with_missing_values(
        pool: &SqlitePool,
