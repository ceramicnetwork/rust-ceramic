...
impl InterfaceUtil {
    ...
    pub fn validate_schema_implementation(
        ...
        let _ = jsonschema::validator_for(expected.as_value()).unwrap();
        let _ = jsonschema::validator_for(implemented.as_value()).unwrap();

        let _expected = expected
            .as_object()
            .ok_or_else(|| anyhow::anyhow!("Interface schema must be an object"))?;
        let _implemented = implemented
            .as_object()
            .ok_or_else(|| anyhow::anyhow!("Model schema must be an object"))?;
