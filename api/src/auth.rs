...
const PREV_REGEX: Lazy<Regex> = Lazy::new(|| Regex::new(r#"prev:(.+)"#).unwrap());
const BISCUIT_REGEX: Lazy<Regex> = Lazy::new(|| Regex::new(r#"biscuit:(.+)"#).unwrap());
...
const DID_METHODS: Lazy<DIDMethods> = Lazy::new(|| {
    ...
    let blocks = cacaos.iter().map(|(cid, sig)| {
        ...
        for res in sig_resources {
            if let Some(res) = PREV_REGEX.captures_iter(&res).next() {
                ...
                let cid =
                    Cid::from_str(&cid).map_err(|_| format!("Invalid previous CID: {cid}"))?;
        ...
        Ok((cid.clone(), Capabilities { resources, biscuit }))
    });
