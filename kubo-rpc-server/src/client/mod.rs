    ...
    C: Has<XSpanIdString> + Clone + Send + Sync + 'static,
{
    ...
    async fn pubsub_ls_post(&self, context: &C) -> Result<PubsubLsPostResponse, ApiError> {
        let mut client_service = self.client_service.clone();
        let mut uri = format!("{}/api/v0/pubsub/ls", self.base_path);
        ...
        match response.status().as_u16() {
            200 => {
                ...
                let body = str::from_utf8(&body)
                    .map_err(|e| ApiError(format!("Response was not valid UTF8: {}", e)))?;
                let body =
                    serde_json::from_str::<models::PubsubLsPost200Response>(body).map_err(|e| {
                        ApiError(format!("Response body did not match the schema: {}", e))
                    })?;
                Ok(PubsubLsPostResponse::Success(body))
            }
            400 => {
                let body = response.into_body();
                let body = body
                ...
                Ok(PubsubLsPostResponse::BadRequest(body))
            }
            code => {
                let headers = response.headers().clone();
                let body = response.into_body().take(100).into_raw().await;
    ...
    async fn pubsub_pub_post(
        ...
        let mut client_service = self.client_service.clone();
        let mut uri = format!("{}/api/v0/pubsub/pub", self.base_path);
        ...
        *request.body_mut() = Body::from(body_string);
        ...
        match response.status().as_u16() {
            200 => Ok(PubsubPubPostResponse::Success),
            400 => {
                let body = response.into_body();
                let body = body
                    .into_raw()
                ...
                Ok(PubsubPubPostResponse::BadRequest(body))
            }
            code => {
                let headers = response.headers().clone();
                let body = response.into_body().take(100).into_raw().await;
    ...
    async fn pubsub_sub_post(
        ...
        let mut client_service = self.client_service.clone();
        let mut uri = format!("{}/api/v0/pubsub/sub", self.base_path);
        ...
        match response.status().as_u16() {
            ...
            400 => {
                ...
                Ok(PubsubSubPostResponse::BadRequest(body))
            }
            code => {
                let headers = response.headers().clone();
                let body = response.into_body().take(100).into_raw().await;
