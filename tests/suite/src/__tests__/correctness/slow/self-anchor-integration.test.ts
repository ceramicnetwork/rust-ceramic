...
// Self-anchoring should complete within 3 minutes (anchor interval + processing time)
const ANCHOR_TIMEOUT_MS = 3 * 60 * 1000
const POLL_INTERVAL_MS = 5000

// Expected chain ID for local Ganache network configured in basic-rust.yaml
// When manually running clay C1 nodes, you can set "eip155:100" with the envvar
const EXPECTED_CHAIN_ID = process.env.CERAMIC_ANCHOR_CHAIN_ID || 'eip155:1337'
...
async function countValidationErrors(
  flightClient: FlightSqlClient,
  streamCid: string,
): Promise<number> {
  const buffer = await flightClient.query(
    `SELECT COUNT(*) as count from event_states
     WHERE cid_string(stream_cid) = '${streamCid}'
     AND NOT array_empty(validation_errors)`,
  )

  const table = tableFromIPC(buffer)
  const row = table.get(0)
  return row ? Number(row.count) : 999
}
...
describe('self-anchoring integration test', () => {
  ...
  test(
    'second node can sync and validate self-anchored events',
    async () => {
      // Ensure both nodes will sync this model
      for (const client of CeramicUrls) {
        await registerInterestMetaModel(client)
        await registerInterest(client, modelStream)
      }
      ...
      const streamCid = documentStream.baseID.cid.toString()
      console.log('Created new document for peer sync', streamCid)

      const peerFlightClient = await createFlightSqlClient({
        ...FLIGHT_OPTIONS,
        host: CeramicFlightEndpoints[1].host,
        port: CeramicFlightEndpoints[1].port,
      })

      const peerAnchorResult = await waitForAnchoredEvent(
        peerFlightClient,
        streamCid,
        ANCHOR_TIMEOUT_MS,
      )

      expect(peerAnchorResult.anchored).toBe(true)
      expect(peerAnchorResult.chainId).toBe(EXPECTED_CHAIN_ID)
      console.log('Peer has discovered first event and anchor')
      ...
      // wait for second anchor
      const startTime = Date.now()
      let newCount = 1
      while (Date.now() - startTime < ANCHOR_TIMEOUT_MS * 2) {
        newCount = await countAnchoredEvents(peerFlightClient, streamCid)
        if (newCount > 1) {
          break
        }
        await delayMs(POLL_INTERVAL_MS)
      ...
      console.log('Peer has discovered second event and anchor')

      // ensure peer node had no issues validating self-anchored events
      const peerValidationErrors = await countValidationErrors(peerFlightClient, streamCid)
      expect(peerValidationErrors).toBe(0)
      console.log('Peer saw no validation failures for self-anchored events')
    },
    ANCHOR_TIMEOUT_MS * 2 + 60000,
  )
})
