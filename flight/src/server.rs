...
async fn run_service(addr: SocketAddr, svr: Router) -> anyhow::Result<()> {
    info!(%addr, "FlightSQL server listening");
    Ok(svr.serve(addr).await?)
}

/// A ConclusionFeed provides access to [`ConclusionEvent`]s.
#[async_trait::async_trait]
pub trait ConclusionFeed: Send + Sync {
    /// Produce a set of conlcusion events up to the limit with an index greater than the highwater_mark
