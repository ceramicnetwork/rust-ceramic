...
use ceramic_store::{CeramicOneEvent, EventInsertableBody, InsertedEvent, SqlitePool};
use cid::Cid;
...
pub struct DeliverableTask {
    ...
    pub(crate) tx_inserted: tokio::sync::mpsc::Sender<InsertedEvent>,
}
...
impl OrderingTask {
    pub async fn run(pool: SqlitePool, q_depth: usize, load_delivered: bool) -> DeliverableTask {
        let (tx_inserted, rx_inserted) = tokio::sync::mpsc::channel::<InsertedEvent>(q_depth);
    ...
    async fn run_loop(
        ...
        mut rx_inserted: tokio::sync::mpsc::Receiver<InsertedEvent>,
    ) {
    ...
    async fn load_by_cid(pool: &SqlitePool, cid: EventCid) -> Result<Option<Self>> {
        ...
        if exists {
            ...
            let known_prev = match parsed_body.header() {
                ceramic_store::EventHeader::Init { cid, .. } => {
                    if !deliverable {
                ...
                ceramic_store::EventHeader::Data { prev, .. }
                | ceramic_store::EventHeader::Time { prev, .. } => {
                    if deliverable {
...
impl From<InsertedEvent> for StreamEvent {
    fn from(ev: InsertedEvent) -> Self {
        match ev.header {
            ceramic_store::EventHeader::Init { cid, .. } => StreamEvent::InitEvent(cid),
            ceramic_store::EventHeader::Data { cid, prev, .. }
            | ceramic_store::EventHeader::Time { cid, prev, .. } => {
...
impl OrderingState {
    ...
    fn add_inserted_events(&mut self, events: Vec<InsertedEvent>) {
        for ev in events {
    ...
    async fn process_undelivered_events_batch(
        ...
        for (event_id, carfile) in event_data {
            ...
            let parsed_body = EventInsertableBody::try_from_carfile(event_cid, &carfile).await?;

            let event = match parsed_body.header() {
                ceramic_store::EventHeader::Init { cid, .. } => {
                    warn!(%cid,"Found init event in database that wasn't previously marked as deliverable. Updating now...");
            ...
            event_cnt += 1;
            self.add_stream_event(parsed_body.header().stream_cid(), event)
        }
...
mod test {
    ...
    async fn get_n_insertable_events(n: usize) -> Vec<EventInsertable> {
        ...
        for event in events {
            let event = EventInsertable::try_new(event.0.to_owned(), &event.1)
                .await
                .unwrap();
            res.push(event);
        }
