...
impl CeramicEventService {
    ...
    async fn insert_events<'a>(
        ...
        if !history_required {
            for ev in &res.inserted {
                ...
                if let Err(e) = self.delivery_task.tx_inserted.try_send(ev.clone()) {
                    match e {
    ...
    pub(crate) async fn load_by_cid(
    ...
    ) -> Result<Option<EventInsertableBody>> {
        ...
        Ok(Some(
            ceramic_store::EventInsertableBody::try_from_carfile(cid, &data).await?,
        ))
    }
}
