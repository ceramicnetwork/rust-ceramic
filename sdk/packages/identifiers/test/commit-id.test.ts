...
describe('constructor', () => {
  ...
  test('unknown streamtype name', () => {
    expect(
      () => new CommitID('invalid-garbage', BASE_CID_STRING, BASE_CID_STRING),
    ).toThrow()
  })
})
...
describe('#atCommit', () => {
  ...
  test('to number 0', () => {
    const traveller = CommitID.fromStream(streamId, 0)
    expect(traveller.commit).toEqual(BASE_CID)
  })
  test('to number 1', () => {
    expect(() =>
      CommitID.fromStream(streamId, 1),
    ).toThrowErrorMatchingSnapshot()
  })
  test('to commit CID', () => {
    const commitId = new CommitID('tile', BASE_CID)
    const traveller = CommitID.fromStream(commitId.baseID, COMMIT_CID)
    expect(traveller.commit).toEqual(COMMIT_CID)
  })
  test('to commit CID as string', () => {
    const commitId = new CommitID('tile', BASE_CID)
    const traveller = CommitID.fromStream(commitId.baseID, COMMIT_CID_STRING)
    expect(traveller.commit).toEqual(COMMIT_CID)
  })
  test('to garbage string', () => {
    expect(() => CommitID.fromStream(streamId, 'garbage')).toThrow()
  })
})
...
describe('nodejs inspect', () => {
  test('init commit', () => {
    const streamid = new CommitID('tile', BASE_CID_STRING)
    expect(inspect(streamid)).toMatchSnapshot()
  })
  test('commit', () => {
    const streamid = new CommitID('tile', BASE_CID_STRING, COMMIT_CID_STRING)
    expect(inspect(streamid)).toMatchSnapshot()
...
test('to primitive', () => {
  ...
  // biome-ignore lint/style/useTemplate: explicit check
  expect(streamid + '').toEqual(streamid.toString())
})
...
test('instanceof', () => {
  ...
  expect(instanceSpy).toHaveBeenCalledWith(commitId)
})
