...
describe('ModelInstanceClient SET Relations', () => {
  ...
  let mockCeramic: any
  let authenticatedDID: any

  // Helper to create multibase encoded data
  const encodeMultibase = (data: any): string => {
      ...
      api: {
        GET: jest.fn().mockResolvedValue({
          data: {
            ...
            event_cid: 'bagcqcerakszw2vsovxznyp5gfnpdj4cqm2xiv76yd24wkjewhhykovorwo6a',
            dimensions: { model: encodeStreamID(StreamID.fromString('kjzl6hvfrbw6c5ynhkxyaqzyllij9mpv9lrnce35d1jlhfdd6mhkp09xcywvt9k')) },
  ...
  describe('createSingleton method with unique values', () => {
    test('should create deterministic instance with unique values', async () => {
      const modelId = StreamID.fromString('kjzl6hvfrbw6c5ynhkxyaqzyllij9mpv9lrnce35d1jlhfdd6mhkp09xcywvt9k')
      
      mockCeramic.postEventType.mockResolvedValueOnce('bagcqcerakszw2vsovxznyp5gfnpdj4cqm2xiv76yd24wkjewhhykovorwo6a' as any)
      ...
      const uniqueBytes = (payload as any).header.unique
    ...
    test('should handle empty unique values', async () => {
      ...
      mockCeramic.postEventType.mockResolvedValue('bagcqcerakszw2vsovxznyp5gfnpdj4cqm2xiv76yd24wkjewhhykovorwo6a' as any)
  ...
  describe('Core logic validation', () => {
    test('createInstance correctly calculates unique values for SET relations', async () => {
      ...
      const content = { field1: 'field1Value', field2: 'field2Value', field3: 'field3Value', extra: 'data' }
      ...
      expect(mockCeramic.postEventType).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          header: expect.objectContaining({
            unique: new TextEncoder().encode('field1Value|field2Value|field3Value')
          })
        })
      )
    })
  ...
  describe('ComposeDB parity tests', () => {
    ...
    test('should handle SET relation fields like Favorite model from ComposeDB', async () => {
      ...
      await client.createInstance({
        ...
        content: {
          docID: 'kjzl6kcym7w8y9zdwz9q2r5bx5srfhdtjvs7ouq2kimk9od0f96v8kp29e37efu',
          tag: 'important',
      ...
      const [, payload] = mockCeramic.postEventType.mock.calls[0]
      const uniqueString = new TextDecoder().decode(payload.header.unique)
      expect(uniqueString).toBe('kjzl6kcym7w8y9zdwz9q2r5bx5srfhdtjvs7ouq2kimk9od0f96v8kp29e37efu|important')
    })
  })
})
